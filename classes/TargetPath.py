...
class TargetPath:
    ...
    def __init__(self, waypoints: StateTensor):
        # waypoints: (M, 3)
    ...
    def normalized_interpolate(self, t: pt.Tensor) -> StateTensor:
    ...
    def distance_interpolate(self, d: pt.Tensor) -> StateTensor:
...
if __name__ == '__main__':
    ...
    x = s
    y = pt.e**(s-1)
    print(pt.stack([x, y, pt.zeros_like(x)], dim=1).shape)
    waypoints = StateTensor.from_tensor(pt.cat([x[:,None], y[:,None], pt.zeros_like(x)[:,None], pt.randn((x.shape[0], 10))], dim=1))
    ...
    print(trajectoryTargets.pos)
    print(trajectoryTargets.vel)
    print(trajectoryTargets.quat)
    print(trajectoryTargets.angvel)

    plt.scatter(*targetPath.waypoints.pos.T[:-1], label='Waypoints')
    plt.plot(*targetPath.waypoints.pos.T[:-1], ls='--', label='Linear Path')
    plt.scatter(*trajectoryTargets.pos.T[:-1], marker='*', label='Step Targets', zorder=2, s=200)
    plt.legend()
    plt.axis('equal')
    plt.show()
